...
local blips = {
	{title="Spor Salonu", colour=5, id=311, x = -1201.2257, y = -1568.8670, z = 4.6101}
}
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(rentbike) do
            ...
            if dist <= 0.5 then
				hintToDisplay('~b~Bisiklet~w~ almak istiyorsanız ~INPUT_CONTEXT~ tuşuna basınız.')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(gym) do
            ...
            if dist <= 0.5 then
				hintToDisplay('~b~Spor Salonu~w~ menüsünü açmak için ~INPUT_CONTEXT~ tuşuna basınız.')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(arms) do
            ...
            if dist <= 0.5 then
				hintToDisplay('~g~Ağırlığı~w~ kaldırmak için ~INPUT_CONTEXT~ tuşuna basınız.')
				
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
					
						TriggerServerEvent('esx_gym:checkChip')
						ESX.ShowNotification("~g~Egzersiz~w~ başlıyor...")
						Citizen.Wait(1000)					
						...
						if membership == true then
							...
							ESX.ShowNotification("Başka bir egzersiz yapmak için ~r~60 saniye ~w~ beklemen gerekli.")
							
							--TriggerServerEvent('esx_gym:trainArms') ## COMING SOON...
							
							training = true
						elseif membership == false then
							ESX.ShowNotification("Eğersiz yapman için ~r~Üyelik kartın~w~ olması gerekli.")
						end
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(chins) do
            ...
            if dist <= 0.5 then
				hintToDisplay('~g~Barfiks~w~ çekmek ~INPUT_CONTEXT~ tuşuna basınız.')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(pushup) do
            ...
            if dist <= 0.5 then
				hintToDisplay('~g~Şınav~w~ çekmek için ~INPUT_CONTEXT~ tuşuna basınız. ')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(yoga) do
            ...
            if dist <= 0.5 then
				hintToDisplay('~g~Yoga~w~ yapmak için ~INPUT_CONTEXT~ tuşuna basınız.')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(situps) do
            ...
            if dist <= 0.5 then
				hintToDisplay('~g~Mekik~w~ çekmek için ~INPUT_CONTEXT~ tuşuna basınız.')
...
function CheckTraining()
	if resting == true then
		ESX.ShowNotification("Biraz ara vermelisin....")
...
function OpenGymMenu()
    ...
    ESX.UI.Menu.Open(
        ...
        {
            title    = 'Spor Salonuna Hoş Geldiniz.',
            elements = {
				{label = 'İçecek almak istiyorum susadım.', value = 'shop'},
				{label = 'Kaç saat açıksınız?', value = 'hours'},
				{label = 'Üyelik almak istiyorum. Ne kadar?', value = 'ship'},
            }
        },
        function(data, menu)
            ...
            elseif data.current.value == 'hours' then
				...
				ESX.ShowNotification("Merhabalar biz ~g~7/24~w~ açığız. Ne zaman isterseniz gelebilirsiniz.")
...
function OpenGymShopMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_shop_menu',
        {
            title    = 'Gym - Shop',
            elements = {
				{label = 'Protein İçeceği ($6)', value = 'protein_shake'},
				{label = 'Su ($1)', value = 'water'},
				{label = 'Hamburger ($2)', value = 'sportlunch'},
				{label = 'Enerji içeceği ($4)', value = 'powerade'},
            }
        },
        function(data, menu)
...
function OpenGymShipMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_ship_menu',
        {
            ...
            elements = {
				{label = 'Üyelik almak istiyorsanız ($800)ödemelisiniz.', value = 'membership'},
            }
        },
        function(data, menu)
